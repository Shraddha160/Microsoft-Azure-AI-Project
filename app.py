...
def narrate():
    ...
    # Synthesize the speech to a stream
    stream = io.BytesIO()
    ...
    # Get the audio from the stream and send it as a response
    stream.seek(0)  # Go to the start of the stream to read it
    return send_file(stream, mimetype='audio/wav', as_attachment=False)
